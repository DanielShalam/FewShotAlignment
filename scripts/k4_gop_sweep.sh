#!/bin/bash
# K=4 sweep with pretrained GOP (PMC-OA, 500K pairs). DINOv3 + BMC-text.
set -u
...
GOP=data/pmc_oa/gop_bmc_to_dv3_n500000_wrapped.pt

DATASETS=(BUSI CHMNIST KneeXray BTMRI COVID_19 CTKidney DermaMNIST Kvasir LungColon OCTMNIST RETINA)
K=4
SEEDS=(42 0 2)
...
run_one () {
  ...
  local tag=${DS}_dv3_bmc_gop_k${K}_s${SEED}
  ...
  CUDA_VISIBLE_DEVICES=$gpu python -u main.py --config configs/busi_biomedclip.yaml \
      ...
      use_op True pretrained_op "$GOP" \
      shots "$K" seed "$SEED" epochs 400 lr 0.00001 \
      > "$log" 2>&1
