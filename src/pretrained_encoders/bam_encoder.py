...
def load_bam_pretrained_weights(model, pretrained_weights, checkpoint_key='model', take_teacher=True):
    ...
    # specifications for adapter
    model.dim = 768
    model.eval()
    train_tfm = T.Compose([
        T.RandomResizedCrop(size=224, scale=(0.5, 1), interpolation=T.InterpolationMode.BICUBIC, antialias=True),
    ...
    return model, train_tfm, eval_tfm
